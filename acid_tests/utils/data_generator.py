...
class DataGenerator:
    ...
    def generate_vectors(self, count: int, 
                        prefix: str = "vec",
                        start_index: int = 0,
                        use_pure_uuid: bool = True) -> List[VectorData]:
        """Generate test vectors with metadata"""
        vectors = []
        
        for i in range(count):
            idx = start_index + i
            # Generate ID based on use_pure_uuid flag
            if use_pure_uuid:
                vector_id = str(uuid.uuid4())
            else:
                vector_id = f"{prefix}-{str(uuid.uuid4())}"
            ...
            vectors.append(VectorData(
                ...
                metadata={
                    ...
                    'tags': self._generate_tags_string(idx),  # Use string instead of list
                    'score': random.uniform(0, 100),
                    'prefix': prefix  # Store prefix in metadata for tracking
                }
            ))
    ...
    def _generate_tags_string(self, index: int) -> str:
        """Generate random tags as comma-separated string for databases that don't support lists"""
        tags = self._generate_tags(index)
        return ','.join(tags)
    ...
    def generate_update_scenarios(self) -> List[Dict[str, Any]]:
        ...
        scenarios.append({
            ...
            'metadata': {
                ...
                'update_time': time.time(),
                'version': 3,
                'tags': 'updated,modified'  # Use string instead of list
            }
        })
